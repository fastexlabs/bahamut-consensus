...
func fixtures() map[string]interface{} {
	...
	executionPayloadWithValueFixtureCapella := &pb.GetPayloadV2ResponseJson{
		ExecutionPayload: &pb.ExecutionPayloadCapellaJSON{
			...
			ActivitiesRoot:    &common.Hash{'e'},
			LogsBloom:         &hexutil.Bytes{'f'},
			TransactionsCount: &hexUint,
			PrevRandao:        &common.Hash{'g'},
			BaseFeePerGas:     "0x123",
			BlockHash:         &common.Hash{'h'},
			Transactions:      []hexutil.Bytes{{'k'}},
			Withdrawals:       []*pb.Withdrawal{},
			BlockNumber:       &hexUint,
			GasLimit:          &hexUint,
			GasUsed:           &hexUint,
...
func Test_GetBlockActivitiesByHash(t *testing.T) {
	t.Run("fetching activities", func(t *testing.T) {

		expectedActivities := &ethpb.BlockActivities{
			BaseFee: 21000000,
			TxCount: 100,
			Activities: []*ethpb.ActivityChange{
				{
					ContractAddress: common.HexToAddress("0xcf8e0d4e9587369b2301d0790347320302cc0943").Bytes(),
					DeltaActivity:   1,
				},
			},
		}
		...
		result, err := service.GetBlockActivitiesByHash(ctx, common.BytesToHash([]byte{'a'}))
		require.NoError(t, err)
		require.NotNil(t, result)
		require.Equal(t, expectedActivities.BaseFee, result.BaseFee)
		require.Equal(t, expectedActivities.TxCount, result.TxCount)
		require.Equal(t, 1, len(expectedActivities.Activities))

		require.Equal(t, expectedActivities.Activities[0].DeltaActivity, result.Activities[0].DeltaActivity)

		expectedContractAddress := common.BytesToAddress(expectedActivities.Activities[0].ContractAddress).String()
		resultContractAddress := common.BytesToAddress(result.Activities[0].ContractAddress).String()
		require.Equal(t, expectedContractAddress, resultContractAddress)

		assert.LogsDoNotContain(t, logHook, "Please update client, detected the following unsupported engine methods:")
	})
}
