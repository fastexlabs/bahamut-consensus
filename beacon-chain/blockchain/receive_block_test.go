...
func TestReceiveBlock_Capella(t *testing.T) {
	// TODO: Find out how can we setup block building for Capella epoch.
	...
	service, tr := minimalTestService(t,
		...
		WithFinalizedStateAtStartUp(st),
		WithExitPool(voluntaryexits.NewPool()),
		WithStateNotifier(&blockchainTesting.MockStateNotifier{RecordEvents: true}))
	ctx := tr.ctx

	activities := make([]uint64, 64)
	activities[0] = 1000
	activities[1] = 1000
	st.SetActivities(activities)

	require.NoError(t, service.saveGenesisData(ctx, st))
	blockActivities, err := mockEngine.GetBlockActivitiesByHash(ctx, common.Hash{})

	require.NoError(t, err)
	blockGenConfig := util.DefaultBlockGenConfig()
	blockGenConfig.ActivitiesRoot = types.DeriveSha(buildExecutionActivityChanges(blockActivities.Activities), trie.NewStackTrie(nil))
	blockGenConfig.NumTransactions = 10

	for i := uint64(1); i < uint64(4)*uint64(cfg.SlotsPerEpoch); i++ {
		b, err := util.GenerateFullBlockCapella(st, keys, blockGenConfig, primitives.Slot(i))
		require.NoError(t, err)

		b.Block.Body.ActivityChanges = blockActivities.Activities
		...
		if err != nil {
			t.Logf("Error: %v, Index: %d", err, i)
		}
		require.NoError(t, err)
	}
	je := service.CurrentJustifiedCheckpt().Epoch
	fe := service.FinalizedCheckpt().Epoch

	require.Equal(t, primitives.Epoch(3), je)
	require.Equal(t, primitives.Epoch(2), fe)
}
