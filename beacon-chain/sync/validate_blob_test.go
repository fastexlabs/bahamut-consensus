...
func TestValidateBlob_InvalidTopicIndex(t *testing.T) {
	...
	require.ErrorContains(t, "/eth2/f170b7df/blob_sidecar_1", err)
	require.Equal(t, result, pubsub.ValidationReject)
}
...
func TestValidateBlob_IncorrectProposerIndex(t *testing.T) {
	...
	require.ErrorContains(t, "expected proposer index 33, got 0", err)
	require.Equal(t, result, pubsub.ValidationReject)
}
...
func TestValidateBlob_EverythingPasses(t *testing.T) {
	...
	b.Message.BlockParentRoot = r[:]
	b.Message.ProposerIndex = 33
	b.Signature, err = signing.ComputeDomainAndSign(beaconState, 0, b.Message, params.BeaconConfig().DomainBlobSidecar, privKeys[33])
	require.NoError(t, err)
