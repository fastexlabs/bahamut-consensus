...
func (s *Service) beaconBlocksByRangeRPCHandler(ctx context.Context, msg interface{}, stream libp2pcore.Stream) error {
	...
	if err != nil {
		...
		s.cfg.p2p.Peers().Scorers().BadResponsesScorer().Increment(stream.Conn().RemotePeer(), fmt.Sprintf("beaconBlocksByRangeRPCHandler() BeaconBlocksByRangeRequest :%v", err))
