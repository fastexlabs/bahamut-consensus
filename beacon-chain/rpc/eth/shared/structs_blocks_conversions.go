...
// todo unit act
// TODO: add Activities, ActivityChanges, TransactionsCount, ActivitiesRoot
func (b *BeaconBlockCapella) ToConsensus() (*eth.BeaconBlockCapella, error) {
	...
	//todo unit act
	payloadActivitiesRoot, err := DecodeHexWithLength(b.Body.ExecutionPayload.ActivitiesRoot, fieldparams.RootLength)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayload.ActivitiesRoot")
	}
	payloadTransactionsCount, err := strconv.ParseUint(b.Body.ExecutionPayload.TransactionsCount, 10, 64)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayload.TransactionsCount")
	}
	payloadLogsBloom, err := DecodeHexWithLength(b.Body.ExecutionPayload.LogsBloom, fieldparams.LogsBloomLength)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayload.LogsBloom")
	...
	return &eth.BeaconBlockCapella{
		...
		Body: &eth.BeaconBlockBodyCapella{
			...
			ExecutionPayload: &enginev1.ExecutionPayloadCapella{
				...
				StateRoot:         payloadStateRoot,
				ReceiptsRoot:      payloadReceiptsRoot,
				ActivitiesRoot:    payloadActivitiesRoot,
				TransactionsCount: payloadTransactionsCount,
				LogsBloom:         payloadLogsBloom,
				PrevRandao:        payloadPrevRandao,
				BlockNumber:       payloadBlockNumber,
				GasLimit:          payloadGasLimit,
...
func (b *BlindedBeaconBlockCapella) ToConsensus() (*eth.BlindedBeaconBlockCapella, error) {
	...
	//todo unit act
	payloadActivitiesRoot, err := DecodeHexWithLength(b.Body.ExecutionPayloadHeader.ActivitiesRoot, fieldparams.RootLength)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayloadHeader.ActivitiesRoot")
	}
	payloadTransactionsCount, err := strconv.ParseUint(b.Body.ExecutionPayloadHeader.TransactionsCount, 10, 64)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayloadHeader.TransactionsCount")
	}
	payloadLogsBloom, err := DecodeHexWithLength(b.Body.ExecutionPayloadHeader.LogsBloom, fieldparams.LogsBloomLength)
	if err != nil {
		return nil, NewDecodeError(err, "Body.ExecutionPayloadHeader.LogsBloom")
	...
	return &BlindedBeaconBlockCapella{
		...
		Body: &BlindedBeaconBlockBodyCapella{
			...
			ExecutionPayloadHeader: &ExecutionPayloadHeaderCapella{
				...
				StateRoot:         hexutil.Encode(b.Body.ExecutionPayloadHeader.StateRoot),
				ReceiptsRoot:      hexutil.Encode(b.Body.ExecutionPayloadHeader.ReceiptsRoot),
				ActivitiesRoot:    hexutil.Encode(b.Body.ExecutionPayloadHeader.ActivitiesRoot),
				TransactionsCount: fmt.Sprintf("%d", b.Body.ExecutionPayloadHeader.TransactionsCount),
				LogsBloom:         hexutil.Encode(b.Body.ExecutionPayloadHeader.LogsBloom),
				PrevRandao:        hexutil.Encode(b.Body.ExecutionPayloadHeader.PrevRandao),
				BlockNumber:       fmt.Sprintf("%d", b.Body.ExecutionPayloadHeader.BlockNumber),
				GasLimit:          fmt.Sprintf("%d", b.Body.ExecutionPayloadHeader.GasLimit),
	...
	return &BeaconBlockCapella{
		...
		Body: &BeaconBlockBodyCapella{
			...
			ExecutionPayload: &ExecutionPayloadCapella{
				...
				StateRoot:         hexutil.Encode(b.Body.ExecutionPayload.StateRoot),
				ReceiptsRoot:      hexutil.Encode(b.Body.ExecutionPayload.ReceiptsRoot),
				ActivitiesRoot:    hexutil.Encode(b.Body.ExecutionPayload.ActivitiesRoot),
				TransactionsCount: fmt.Sprintf("%d", b.Body.ExecutionPayload.TransactionsCount),
				LogsBloom:         hexutil.Encode(b.Body.ExecutionPayload.LogsBloom),
				PrevRandao:        hexutil.Encode(b.Body.ExecutionPayload.PrevRandao),
				BlockNumber:       fmt.Sprintf("%d", b.Body.ExecutionPayload.BlockNumber),
				GasLimit:          fmt.Sprintf("%d", b.Body.ExecutionPayload.GasLimit),
...
func DepositsToConsensus(src []*Deposit) ([]*eth.Deposit, error) {
	...
	for i, d := range src {
		...
		contract, err := DecodeHexWithLength(d.Data.Contract, fieldparams.ContractAddressLength)
		if err != nil {
			return nil, NewDecodeError(err, fmt.Sprintf("[%d].Contract", i))
		}
	...
	for i, d := range src {
		...
		deposits[i] = &Deposit{
			...
			Data: &DepositData{
				...
				Contract:              hexutil.Encode(d.Data.Contract),
			},
		}
	}
	return deposits, nil
