...
func TestGetAttesterDuties(t *testing.T) {
	...
	t.Run("single validator", func(t *testing.T) {
		...
		assert.Equal(t, "117", duty.ValidatorCommitteeIndex)
	})
	...
	t.Run("next epoch", func(t *testing.T) {
		...
		duty := resp.Data[0]
		assert.Equal(t, "0", duty.CommitteeIndex)
		assert.Equal(t, "48", duty.Slot)
		assert.Equal(t, "0", duty.ValidatorIndex)
		assert.Equal(t, hexutil.Encode(pubKeys[0]), duty.Pubkey)
		assert.Equal(t, "128", duty.CommitteeLength)
		assert.Equal(t, "1", duty.CommitteesAtSlot)
		assert.Equal(t, "115", duty.ValidatorCommitteeIndex)
	})
...
func TestGetProposerDuties(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		require.Equal(t, primitives.ValidatorIndex(937), vid)
		require.NotNil(t, expectedDuty, "Expected duty for slot 11 not found")
		assert.Equal(t, "937", expectedDuty.ValidatorIndex)
		assert.Equal(t, hexutil.Encode(pubKeys[937]), expectedDuty.Pubkey)
	})
	t.Run("next epoch", func(t *testing.T) {
		bs, err := transition.GenesisBeaconState(context.Background(), deposits, 0, eth1Data)
		require.NoError(t, err, "Could not set up genesis state")
		...
		require.Equal(t, true, has)
		require.Equal(t, primitives.ValidatorIndex(2309), vid)
		require.NotNil(t, expectedDuty, "Expected duty for slot 43 not found")
		assert.Equal(t, "2309", expectedDuty.ValidatorIndex)
		assert.Equal(t, hexutil.Encode(pubKeys[2309]), expectedDuty.Pubkey)
	})
	t.Run("prune payload ID cache", func(t *testing.T) {
		...
		require.Equal(t, true, has)
		require.Equal(t, primitives.ValidatorIndex(362), vid)
	})
	t.Run("epoch out of bounds", func(t *testing.T) {
		bs, err := transition.GenesisBeaconState(context.Background(), deposits, 0, eth1Data)
		require.NoError(t, err, "Could not set up genesis state")
