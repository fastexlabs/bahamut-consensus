...
// todo unit act
func (s *Server) GetSharedActivities(w http.ResponseWriter, r *http.Request) {
	...
	sharedAct := st.SharedActivity()
	if sharedAct == nil {
		http2.HandleError(w, "Nil shared activity in state", http.StatusInternalServerError)
		return
	}
	sharedActivity := &SharedActivities{
		TransactionsGas: sharedAct.TransactionsGasPerPeriod,
		BaseFee:         sharedAct.BaseFeePerPeriod,
	}

	resp := &GetSharedActivitiesResponse{
		Data:                sharedActivity,
		ExecutionOptimistic: isOptimistic,
		Finalized:           isFinalized,
	}
