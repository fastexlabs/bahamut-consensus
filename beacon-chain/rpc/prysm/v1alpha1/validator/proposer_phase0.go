...
func (vs *Server) getPhase0BeaconBlock(ctx context.Context, req *ethpb.BlockRequest) (*ethpb.BeaconBlock, error) {
	...
	blk := &ethpb.BeaconBlock{
		...
		Body: &ethpb.BeaconBlockBody{
			...
			ActivityChanges:      blkData.ActivityChanges,
			LatestProcessedBlock: blkData.LatestProcessedBlock,
			TransactionsCount:    blkData.TransactionsCount,
...
func (vs *Server) buildPhase0BlockData(ctx context.Context, req *ethpb.BlockRequest) (*blockData, error) {
	...
	activityChanges, transactionsCount, latestProcesseBlock, err := vs.activityChanges(ctx, head)
	if err != nil {
		return nil, err
	}
	...
	return &blockData{
		...
		LatestProcessedBlock: latestProcesseBlock,
