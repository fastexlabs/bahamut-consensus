...
func convertToBlockContainer(blk interfaces.ReadOnlySignedBeaconBlock, root [32]byte, isCanonical bool) (*ethpb.BeaconBlockContainer, error) {
	...
	case version.FastexPhase1:
		if blk.IsBlinded() {
			rBlk, err := blk.PbBlindedFastexPhase1Block()
			if err != nil {
				return nil, err
			}
			ctr.Block = &ethpb.BeaconBlockContainer_BlindedFastexPhase1Block{BlindedFastexPhase1Block: rBlk}
		} else {
			rBlk, err := blk.PbFastexPhase1Block()
			if err != nil {
				return nil, err
			}
			ctr.Block = &ethpb.BeaconBlockContainer_FastexPhase1Block{FastexPhase1Block: rBlk}
		}
