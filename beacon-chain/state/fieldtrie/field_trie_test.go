...
func TestFieldTrie_CopyTrieImmutable(t *testing.T) {
	...
	trie, err := fieldtrie.NewFieldTrie(stateTypes.FieldIndex(15), stateTypes.BasicArray, newState.RandaoMixes(), uint64(params.BeaconConfig().EpochsPerHistoricalVector))
	require.NoError(t, err)
...
func TestFieldTrie_CopyAndTransferEmpty(t *testing.T) {
	trie, err := fieldtrie.NewFieldTrie(stateTypes.FieldIndex(15), stateTypes.BasicArray, nil, uint64(params.BeaconConfig().EpochsPerHistoricalVector))
	require.NoError(t, err)
