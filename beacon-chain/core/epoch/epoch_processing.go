...
func ProcessEffectiveActivityUpdates(state state.BeaconState) (state.BeaconState, error) {
	...
	validatorFunc := func(idx int, val *ethpb.Validator) (bool, *ethpb.Validator, error) {
		...
		activity := ((val.EffectiveActivity + activities[idx])*uint64(params.BeaconConfig().EpochsPerActivityPeriod) - val.EffectiveActivity)/uint64(params.BeaconConfig().EpochsPerActivityPeriod)
		newVal := ethpb.CopyValidator(val)
...
// ProcessActivityReset reset current validators' activities slice.
...
// ProcessTransactionsGasPerPeriodUpdate process transaction gas per period update during epoch processing.
func ProcessTransactionsGasPerPeriodUpdate(state state.BeaconState) (state.BeaconState, error) {
	txsPerPeriod := ((state.TransactionsGasPerPeriod() + state.TransactionsPerLatestEpoch()*21000)*uint64(params.BeaconConfig().EpochsPerActivityPeriod) - state.TransactionsGasPerPeriod())/uint64(params.BeaconConfig().EpochsPerActivityPeriod)
...
// ProcessNonStakersGasPerPeriodUpdate process non stakers gas per period update during epoch processing.
func ProcessNonStakersGasPerPeriodUpdate(state state.BeaconState) (state.BeaconState, error) {
	nonStakersGasPerPeriod := ((state.NonStakersGasPerPeriod() + state.NonStakersGasPerEpoch())*uint64(params.BeaconConfig().EpochsPerActivityPeriod) - state.NonStakersGasPerPeriod())/uint64(params.BeaconConfig().EpochsPerActivityPeriod)
