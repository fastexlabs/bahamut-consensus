...
func configureTestnet(ctx *cli.Context) error {
	if ctx.Bool(FastexChainTestnet.Name) {
		log.Warn("Running on the Fastex Chain Testnet")
		if err := params.SetActive(params.FastexChainTestnetConfig().Copy()); err != nil {
			return err
		}
		params.UseFastexChainTestnetConfig()
